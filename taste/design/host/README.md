# Host Design Philosophy

This directory owns Canvas host design philosophy and scenario judgment.

Read [`HOST.md`](./HOST.md) when changing host chrome, sidebar views, search, settings, gallery controls, theme controls, dense operational rows, or host UI adapter consumption.

Use other routes for other questions:

- [`../DESIGN.md`](../DESIGN.md) owns global Canvas design taste.
- `apps/docs/content/docs/canvas/host/index.mdx` owns host architecture, metadata, import boundaries, and runtime behavior.
- `todo/host-component-pipeline-roadmap.md` owns migration slices and remaining host component pipeline work.

Historical design material in `_archive/design` is comparison only. Do not use it as current host architecture or product structure.
