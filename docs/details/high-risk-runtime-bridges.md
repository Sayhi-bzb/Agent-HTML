...
- 为什么这三类组件仍然是当前兼容层里的高风险点
- 旧 authoring 字段现在具体退到了哪一层
- 哪些测试和 gate 仍在保护这些兼容桥

## 结论先行

这三类组件之所以危险，不是因为公开 contract 仍未切换，而是因为：
...
如果现在只是判断现实优先级，直接看本文即可。  
这里已经是单组件 bridge 现实的统一入口。

## 7. 当前总判断

`tabs`、`accordion`、`table` 当前都已经不是“公开 contract 仍未切换”的问题。
