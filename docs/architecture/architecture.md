...
- 当前稳定的配置选择入口是 `profile-ref`。
- `gallery` 当前用于配置和预览 artifact profile，不属于语义使用层，也不是 runtime host 本身。
...
- `spacing scale` 与 `shadow scale` 仍是设计词汇，当前还没有写入稳定 artifact profile

### 全局layout

- `frame`、`measure`、`rhythm`、`density posture`、`partition`、`reflow`
- 这些已经作为稳定配置子树进入 artifact profile。
- 当前稳定实现里分别落在 `globalLayout` 和 `componentLayout`，由 layout projection 消费。
...
- 旧 `style-ref` 已退出主公开协议；当前若输入会触发明确错误，而不是成功 fallback。
- 详见 [schema.md](./schema.md)。

#### layout组件：页面的关系积木

- 下面的 `stack:measure/rhythm/density` 这类记号描述的是当前稳定配置分工，不是 agent-facing props。
...
- `DocumentArtifactShell` 当前已收口成 artifact root；文档型默认值现在主要下沉到 document layout policy，例如全局宽度、padding 与 section spacing
- React
- Vite
- Tailwind
- shadcn
