...
describe("agent html block layout transitions", () => {
  it("computes FLIP deltas from previous to next rects", () => {
    expect(
      getAgentHtmlBlockLayoutTransition({
        next: snapshot("/Cell/Block[1]", "B", {
          height: 50,
          left: 40,
          top: 80,
          width: 100,
        }),
        previous: snapshot("/Cell/Block[0]", "B", {
          height: 100,
          left: 10,
          top: 20,
          width: 200,
  ...
  it("skips visually equivalent rects", () => {
    expect(
      getAgentHtmlBlockLayoutTransition({
        next: snapshot("/Cell/Block[0]", "A", { left: 10, top: 10 }),
        previous: snapshot("/Cell/Block[0]", "A", { left: 10.2, top: 10.2 }),
      })
    ).toBeNull()
  })
  ...
  it("matches moved blocks by unique motion key before path", () => {
    const transitions = getAgentHtmlBlockLayoutTransitions({
      next: [
        snapshot("/Cell/Block[0]", "B", { top: 0 }),
        snapshot("/Cell/Block[1]", "A", { top: 120 }),
      ],
      previous: [
        snapshot("/Cell/Block[0]", "A", { top: 0 }),
        snapshot("/Cell/Block[1]", "B", { top: 120 }),
      ],
    })

    expect(transitions.get("/Cell/Block[0]")).toMatchObject({
      deltaY: 120,
    })
    expect(transitions.get("/Cell/Block[1]")).toMatchObject({
      deltaY: -120,
    })
  })
  ...
  it("falls back to path when motion keys are not unique", () => {
    const transitions = getAgentHtmlBlockLayoutTransitions({
      next: [snapshot("/Cell/Block[0]", "duplicate", { top: 80 })],
      previous: [
        snapshot("/Cell/Block[0]", "duplicate", { top: 10 }),
        snapshot("/Cell/Block[1]", "duplicate", { top: 160 }),
      ],
    })
