...
import type {
  ...
  KanbanItemData,
  KanbanItemElement,
} from "@/agent-html/runtime/ui/kanban-types"
...
function Kanban({ children }: { children?: React.ReactNode }) {
  ...
  const dragStartPositionRef = React.useRef<{
    columnValue: string
    index: number
  } | null>(null)
  ...
  function handleDragStart(event: DragStartEvent) {
    ...
    dragStartPositionRef.current = findItemPosition(columns, itemValue)
  ...
  function handleDragOver(event: DragOverEvent) {
    ...
    setColumns((currentColumns) =>
      moveKanbanItem(currentColumns, String(active.id), String(over.id), arrayMove)
    )
  }
  ...
  function handleDragEnd() {
    ...
    if (itemValue) {
      ...
      const nextPosition = findItemPosition(columns, itemValue)

      if (
        previousPosition &&
        nextPosition &&
        (previousPosition.columnValue !== nextPosition.columnValue ||
          previousPosition.index !== nextPosition.index)
      ) {
        dispatchAgentHtmlInteractionEvent({
          itemValue,
          kind: "kanban_item_moved",
          nextColumnValue: nextPosition.columnValue,
          nextIndex: nextPosition.index,
          previousColumnValue: previousPosition.columnValue,
          previousIndex: previousPosition.index,
        })
      }
    }
