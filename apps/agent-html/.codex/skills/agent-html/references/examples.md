<!-- AUTO-GENERATED FROM src/agent-html/fixtures -->
# Agent-HTML Examples

Source fixtures live in:

- `src/agent-html/fixtures/valid/`
- `src/agent-html/fixtures/invalid/`

Use the closest valid fixture before inventing new structure.

## Valid Patterns

- `button-basic.xml`
- `card-tabs-grid.xml`
- `codeblock-basic.xml`
- `complex-dashboard.xml`
- `icon-basic.xml`
- `image-basic.xml`
- `kanban-basic.xml`
- `minimal-page.xml`
- `section-width.xml`
- `text-basic.xml`
- `timeline-basic.xml`

## Invalid Patterns

- `bare-text-under-grid.xml`
- `bare-text-under-section.xml`
- `button-icon-missing-label.xml`
- `button-invalid-child.xml`
- `button-invalid-href.xml`
- `button-invalid-variant.xml`
- `button-size-not-allowed.xml`
- `carousel-missing-content.xml`
- `chart-missing-series.xml`
- `codeblock-empty.xml`
- `codeblock-invalid-language.xml`
- `codeblock-missing-language.xml`
- `image-invalid-fit.xml`
- `image-invalid-src.xml`
- `image-with-child.xml`
- `kanban-column-missing-title.xml`
- `kanban-column-missing-value.xml`
- `kanban-duplicate-column-value.xml`
- `kanban-duplicate-item-value.xml`
- `kanban-invalid-child.xml`
- `kanban-item-missing-value.xml`
- `kanban-missing-column.xml`
- `layout-gap-not-allowed.xml`
- `missing-tabs-trigger-value.xml`
- `section-invalid-width.xml`
- `text-with-child.xml`
- `timeline-invalid-status.xml`
- `timeline-item-missing-title.xml`
- `timeline-missing-item.xml`
- `timeline-unknown-icon.xml`
- `unknown-icon-name.xml`
- `unknown-tag.xml`
