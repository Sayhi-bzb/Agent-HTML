...
resources. It is not an installation target. The dev host resolves those
workspace imports through CLI-owned dependencies, exact package aliases, and
Vite import-entry resolution. React and React DOM stay pinned through dedicated
aliases so the host and artifacts share one renderer instance.
...
installs. `dev-host.test.mjs` checks Vite dependency aliases, ESM import entries,
and fresh-project startup.
