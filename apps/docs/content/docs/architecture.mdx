...
The dependency contract is executable: `boundaries.test.mjs` checks source-only
workspace files, source metadata, CLI runtime ownership, and forbidden vendored
installs. `routes.test.mjs` checks route pipeline ownership. `dev-host.test.mjs`
checks Vite dependency aliases, ESM import entries, runtime exports,
React singleton resolution, artifact rendering, and fresh-project startup.
