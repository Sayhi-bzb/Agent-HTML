...
apps/desktop
  -> project selection, session state, runtime supervision, preferences,
     packaged Node, and Desktop chrome
```
...
| Desktop session | `apps/desktop/src-tauri` + `apps/desktop/src/session.ts` | selected folder, initialization choice, preferences | one supervised workspace runtime | artifact layout, Host route semantics, project preferences, multiple active runtimes | Desktop Vitest and Cargo tests |
