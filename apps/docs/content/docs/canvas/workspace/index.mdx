...
`components.json`, `tsconfig.json`, and `package.json` stay at the workspace
root because shadcn, TypeScript, editors, and dependency tooling discover them
from there. `.agent-html/package.json` is source metadata only; the workspace
does not keep `package-lock.json`, `manifest.json`, or `node_modules`.
