...
already recorded in `packages/cli/src/react-canvas/boundaries.test.mjs`; do not
add new arbitrary values when a Canvas semantic class or ordinary layout utility
can express the same constraint.
